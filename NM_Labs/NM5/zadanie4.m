...
[X_coarse, Y_coarse] = meshgrid(x_coarse, y_coarse); % siatka dwuwymiarowa dla równomiernie rozmieszczonych punktów

% Wyznaczenie wartości funkcji oryginalnej w węzłach
F_coarse = cos(3*pi*X_coarse) .* cos(2*pi*Y_coarse); % zadanie A: zmień definicję funkcji interpolowanej
...
ylabel('y');
zlabel('F(x, y)');
title('Wartości funkcji');

subplot(2, 1, 2);
surf(X_fine, Y_fine, F_fine);
xlabel('x');
ylabel('y');
zlabel('F(x, y) po interpolacji');
